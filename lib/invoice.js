...
export function splitAddress(address) {
  ...
  // If address length is 90 or less, return as single line
  if (address.length <= 90) {
    return { line1: address, line2: "" };
  }

  // Address exceeds 90 characters, need to split
  ...
  if (address.includes(",")) {
    ...
    // Find optimal split point to keep line1 under 90 chars
    let line1Parts = [];
    let line2Parts = [];
    let currentLength = 0;
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (
        currentLength + partWithComma.length <= 90 &&
        line2Parts.length === 0
      ) {
  ...
  if (address.includes(" ")) {
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (
        currentLength + partWithSpace.length <= 90 &&
        line2Parts.length === 0
      ) {
        line1Parts.push(parts[i]);
  ...
  // If no delimiter and exceeds 90, split at 90 characters
  return {
    line1: address.substring(0, 90),
    line2: address.substring(90),
  };
}
...
function convertInvoiceNumber(invoiceNo, invoiceDate) {
  ...
  const istDate = new Date(invoiceDate.toLocaleString("en-US", { timeZone: "Asia/Kolkata" }));
...
function convertCreditNoteNumber(creditNoteNo, creditNoteDate) {
  ...
  const istDate = new Date(creditNoteDate.toLocaleString("en-US", { timeZone: "Asia/Kolkata" }));
  const year = istDate.getFullYear();
  const month = String(istDate.getMonth() + 1).padStart(2, "0");
...
export function generateCreditNoteJSONB(params) {
  ...
  const istCreditNoteDate = new Date(creditNoteDate.toLocaleString("en-US", { timeZone: "Asia/Kolkata" }));
  const formattedCreditNoteDate = `${String(istCreditNoteDate.getDate()).padStart(
    2,
    "0"
  )}/${String(istCreditNoteDate.getMonth() + 1).padStart(
    2,
    "0"
  )}/${istCreditNoteDate.getFullYear()}`;

  const istOriginalInvoiceDate = new Date(originalInvoiceDate.toLocaleString("en-US", { timeZone: "Asia/Kolkata" }));
