 ...
 * If address length exceeds 90 characters, splits based on comma or space
...
export function splitAddress(address) {
  ...
  const splitByDelimiter = (parts, joiner) => {
    let line1Parts = [];
    let line2Parts = [];
    let currentLength = 0;
    let splitDone = false;
    ...
    for (let i = 0; i < parts.length; i++) {
      const partWithJoiner = i === 0 ? parts[i] : joiner + parts[i];
      if (!splitDone && currentLength + partWithJoiner.length <= 100) {
        line1Parts.push(parts[i]);
        currentLength += partWithJoiner.length;
      } else {
        splitDone = true;
        line2Parts.push(parts[i]);
      }
    }
    return { line1: line1Parts.join(joiner), line2: line2Parts.join(joiner) };
  };

  if (address.includes("\n")) {
    const parts = address.split("\n").filter((p) => p.trim());
    return {
      line1: parts[0].substring(0, 100),
      line2: parts.slice(1).join(", ").substring(0, 100),
    };
  }
  ...
  if (address.includes(",")) {
    const parts = address
      ...
      .map((p) => p.trim())
      .filter((p) => p);
    const result = splitByDelimiter(parts, ",");
    // ✅ Only use comma split if line1 actually got something
    if (result.line1.length > 0) {
      return {
        line1: result.line1.substring(0, 100),
        line2: result.line2.substring(0, 100),
      };
    }
    // ❌ First comma chunk was itself > 100 chars, fall through to space split
  }

  // Space split — always works as final delimiter fallback
  if (address.includes(" ")) {
  ...
  // Hard cut if no delimiters at all
  return {
    line1: address.substring(0, 100),
    line2: address.substring(100, 200),
  };
}
...
function convertInvoiceNumber(invoiceNo, invoiceDate) {
  ...
  // Format: INV-YYYY-MM-XXXX (using IST timezone)
  const istDate = new Date(
    invoiceDate.toLocaleString("en-US", { timeZone: "Asia/Kolkata" })
  );
...
export function generateInvoiceJSONB(params) {
  ...
  // Format date as DD/MM/YYYY (using IST timezone)
  const istDate = new Date(
    invoiceDate.toLocaleString("en-US", { timeZone: "Asia/Kolkata" })
  );
...
function convertCreditNoteNumber(creditNoteNo, creditNoteDate) {
  ...
  // Format: CRN-YYYY-MM-XXXX (using IST timezone)
  const istDate = new Date(
    creditNoteDate.toLocaleString("en-US", { timeZone: "Asia/Kolkata" })
  );
  const year = istDate.getFullYear();
  const month = String(istDate.getMonth() + 1).padStart(2, "0");
...
export function generateCreditNoteJSONB(params) {
  ...
  // Format dates as DD/MM/YYYY (using IST timezone)
  const istCreditNoteDate = new Date(
    creditNoteDate.toLocaleString("en-US", { timeZone: "Asia/Kolkata" })
  );
  const formattedCreditNoteDate = `${String(
    istCreditNoteDate.getDate()
  ).padStart(2, "0")}/${String(istCreditNoteDate.getMonth() + 1).padStart(
    2,
    "0"
  )}/${istCreditNoteDate.getFullYear()}`;

  const istOriginalInvoiceDate = new Date(
    originalInvoiceDate.toLocaleString("en-US", { timeZone: "Asia/Kolkata" })
  );
  const formattedOriginalInvoiceDate = `${String(
    istOriginalInvoiceDate.getDate()
  ).padStart(2, "0")}/${String(istOriginalInvoiceDate.getMonth() + 1).padStart(
    2,
    "0"
  )}/${istOriginalInvoiceDate.getFullYear()}`;
