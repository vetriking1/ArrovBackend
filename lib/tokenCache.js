...
export function isTokenError(errorData) {
  ...
  // Check root level error messages
  const errorMessage = (
  ...
  // Check ErrorDetails array (common in e-invoice API responses)
  if (errorData.ErrorDetails && Array.isArray(errorData.ErrorDetails)) {
    return errorData.ErrorDetails.some(err => {
      const errMsg = (err.ErrorMessage || err.errorMessage || err.message || '').toLowerCase();
      const errCode = err.ErrorCode || err.errorCode || '';
      // Error code 1005 is "Invalid Token" in e-invoice API
      return errCode === '1005' || tokenErrorKeywords.some(keyword => errMsg.includes(keyword));
    });
  }
  
  // Check error code at root level
  if (errorData.ErrorCode === '1005' || errorData.errorCode === '1005') {
    return true;
  }
